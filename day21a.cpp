...
void dfs(vector<int> &ssDist, vector<vector<int>> &distController, vector<vector<pair<int,int>>> &adj, vector<bool> &visited, vector<int> &controllerSeq, int curr) {
...
int main() {
  ...
  vector<vector<int>> self(5, vector<int>(5, 1));
  ...
  auto calcDist = [&](vector<vector<int>> &distController, vector<vector<pair<int,int>>> &adj) -> vector<vector<int>> {
    int n = adj.size();
    vector<vector<int>> dist(n, vector<int>(n, INF));

    vector<bool> visited(n);
    for (int i = 0; i < n; ++i) {
      vector<int> &ssDist = dist[i]; // single source dist
      ssDist[i] = 1;
  ...
  auto robotA = calcDist(self, dirAdj);
  auto robotB = calcDist(robotA, dirAdj);
  auto numpad = calcDist(robotB, numAdj);
